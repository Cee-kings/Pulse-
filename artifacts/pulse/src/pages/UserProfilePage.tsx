...
import { ArrowLeft, Heart, Clock, Wallet, ExternalLink, Copy, Check } from "lucide-react";
import { getAuthorByUsername, getPostsByAuthor } from "../data/mockData";
...
export default function UserProfilePage() {
  ...
  if (!author) {
    return (
      <div className="max-w-2xl mx-auto px-6 py-20 text-center animate-fade-up">
        <div className="w-16 h-16 mx-auto rounded-2xl flex items-center justify-center mb-6"
          style={{ background: "rgba(139,92,246,0.12)", border: "1px solid rgba(139,92,246,0.2)" }}>
          <span className="text-3xl">@</span>
        </div>
        <h1 className="text-2xl font-bold text-foreground mb-3">@{username} not found</h1>
        <p className="text-muted-foreground text-sm mb-6 leading-relaxed">
          No writer with that username exists on Pulse yet.
        </p>
        ...
        {author.walletId && (
          <div className="mt-4 inline-flex items-center gap-3 rounded-xl px-4 py-2.5"
            ...
            <div className="flex items-center gap-2 ml-1">
              <button onClick={copyWallet} className="text-muted-foreground/50 hover:text-muted-foreground transition-colors" title="Copy address">
