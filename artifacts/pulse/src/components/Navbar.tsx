...
import { PenLine, ChevronDown, LogOut, User, Sparkles, Wallet, Pencil } from "lucide-react";
import { useAuth, shortAddress } from "../hooks/useAuth";

export default function Navbar() {
  const [location] = useLocation();
  const { user, logout, openUsernamePrompt } = useAuth();
          ...
          {user && (
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1.5 sm:gap-2 pl-1 pr-2 py-1 rounded-full transition-all hover:bg-white/5"
              ...
              {menuOpen && (
                <>
                  ...
                  <div
                    className="absolute right-0 top-full mt-2 w-56 rounded-2xl z-50 overflow-hidden py-2"
                    style={{
                  ...
                  >
                    ...
                    <button
                      onClick={() => { setMenuOpen(false); openUsernamePrompt(); }}
                      className="w-full flex items-center gap-2.5 px-4 py-2 text-sm text-muted-foreground hover:text-foreground transition-colors"
